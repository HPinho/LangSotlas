...
class TestCodegenEnhancements(unittest.TestCase):
    def test_auto_type_inference(self):
        ...
        """
        c = codegen(src)
        self.assertIn("const __auto_type x = 42;", c)
        self.assertIn("__auto_type y = 100;", c)
        self.assertNotIn("/* auto */", c)

    def test_closure_codegen(self):
        src = """module x;
        fn f() -> Void {
        ...
        """
        c = codegen(src)
        self.assertIn("__sotlas_closure_0", c)
        self.assertIn("static int64_t __sotlas_closure_0", c)
        self.assertIn("(&__sotlas_closure_0)", c)

    def test_class_vtable_codegen(self):
        src = """module x;
        class Shape {
            var id: Int64;
            moldable fn area() -> Int64 { return 0; }
        }
        """
        c = codegen(src)
        self.assertIn("struct ShapeVtable {", c)
        self.assertIn("Shape__vtable_default", c)
        self.assertIn("Shape__init_vtable", c)
        self.assertIn("const ShapeVtable* vtable;", c)
