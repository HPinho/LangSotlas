...
class TestSemaSRGAdvanced(unittest.TestCase):
    def test_topology_mismatch_raises_error(self):
        src = """module mem;
        fn map_io(p: *rawphys UInt32) -> Void {
            let v: *virtmap UInt32 = p;
        }
        """
        with self.assertRaises(SotlasSemaError) as ctx:
            parse_check(src)
        self.assertIn("topologia incompatível", str(ctx.exception))
        self.assertIn("*rawphys", str(ctx.exception))
        self.assertIn("*virtmap", str(ctx.exception))

    def test_handover_blocks_use_after_move(self):
        src = """module srg;
        struct Buffer { var addr: UInt64; }
        fn consume(b: sole Buffer) -> Void {
            handover b;
            let again = b;
        }
        """
        with self.assertRaises(SotlasSemaError) as ctx:
            parse_check(src)
        self.assertIn("após transferência", str(ctx.exception))


class TestSemaDiagnostics(unittest.TestCase):
    def test_caret_diagnostic_formatting(self):
        src = "module err;\nfn f() -> Void {\n    let x = missing_symbol;\n}"
        tokens = Lexer(src, "sample.sotlas").tokenize()
        ast = Parser(tokens, "sample.sotlas").parse()
        sema = Sema(ast, "sample.sotlas", source=src)
        with self.assertRaises(SotlasSemaError) as ctx:
            sema.check()
        err_str = str(ctx.exception)
        self.assertIn("error: símbolo não declarado: 'missing_symbol'", err_str)
        self.assertIn("sample.sotlas:3:", err_str)
        self.assertIn("let x = missing_symbol;", err_str)
